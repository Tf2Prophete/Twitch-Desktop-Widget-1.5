...
Global $MainGui, $CurrentGui, $PositionGui, $OptionsGui, $UpdateButton, $UpdatingButton, $CurrentStreams = 1, $GetNumber, $GetNumber2
Global $StopUpdatingInfo = 0

Global $OnlineColor = "0x00FF3C", $OfflineColor = "0xFF0000"

Dim $StreamerButton[30], $Split[1000]
...
Next
	...
	GuiCtrlCreateLabel("Prophete's Twitch Widget", 41, 0, 200, 15)
    ...
    Local $ContextMenuExitItem = GUICtrlCreateMenuItem("Exit...", $ContextMenu)
	GUICtrlSetOnEvent(-1, "_Exit")
    Local $ContextMenuUpdateItem = GUICtrlCreateMenuItem("Update...", $ContextMenu)
	GUICtrlSetOnEvent(-1, "_Update")
    GuiSetState()
	$CurrentStreams = 1
	_Update()
...
Func _CreateOptionsGui()
	...
	GuiSetFont(13)
	$ManualUpdateButton = GuiCtrlCreateButton("Manual Update", 55, 45, 180, 30)
	GUICtrlSetOnEvent(-1, "_ManualUpdateSet")
	$AutomaticUpdateButton = GuiCtrlCreateButton("Automatic Update", 55, 85, 180, 30)
	GUICtrlSetOnEvent(-1, "_AutomaticUpdateSet")
	$ChangeTimerButton = GuiCtrlCreateButton("Change Update Timer", 55, 125, 180, 30)
...
Func _UpdateStreamer()
	...
	GuiDelete($OptionsGui)
		...
		$SplitData = StringSplit($StreamData, "|")
			...
			Sleep(10)
				EndIf
				If $StopUpdatingInfo = 0 Then
			$InputName = InputBox("Stream Update", "Please input the name you wish to show on the button.", $SplitData[1])
			$InputLink = InputBox("Stream Update", "Please input the link to the Twitch.tv stream you wish to associate with this button.", "Www.twitch.tv/" & $Split[2])
			$Split = StringSplit($InputLink, "tv/", 1)
			If $Split[1] = "http://twitch." OR "www.twitch." Then
			$CheckSave = MsgBox(4, "Stream Update", "Is this information correct?" & @CRLF & @CRLF & "Stream Name: " & $InputName & @CRLF & "Stream Link: " & $InputLink)
			If $CheckSave = 6 Then
		...
		EndIf
		Else
		MsgBox(48, "Error", "There is no stream attached to this button yet. Please use the 'Add Streamer' function before using the edit.")
				EndIf
				$StopUpdatingInfo = 0
	GuiDelete($MainGui)
	_CreateMainGUI()
...
Func _RunStream()
	...
	$GetNumber2 = StringTrimLeft($Info, 9)

	$CheckEdit = InputBox($GetNumber2, "What would you like to do?" & @CRLF & @CRLF & "1 = Add New Stream" & @CRLF & "2 = Edit Current Stream" & @CRLF & "3 = Run Current Stream" & @CRLF & "4 = Run Chat for Current Stream")
	If $CheckEdit = 1 Then
		_UpdateStreamer()
	ElseIf $CheckEdit = 2 Then
		_UpdateStreamer()
	ElseIf $CheckEdit = 4 Then
				If StringInStr($Info, "Streamer ") Then
			MsgBox(48, "Error", "Please select 'Add New Stream' before trying to run a chat!")
		Else
		$StreamData = IniRead(@ScriptDir & "/Data/Preferences.ini", "Streams", $GetNumber[1], "NA")
		$SplitData = StringSplit($StreamData, "|")
		$Split = StringSplit($SplitData[2], "streams/", 1)
		ShellExecute("http://www.twitch.tv/" & $Split[2] & "/chat?popout=")
		EndIf
	ElseIf $CheckEdit = 3 Then
		If StringInStr($Info, "Streamer ") Then
			MsgBox(48, "Error", "Please select 'Add New Stream' before trying to run a stream!")
		Else
